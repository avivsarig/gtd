...
down_revision: Union[str, None] = '03363227c007'
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('next', 'waiting', 'someday', 'completed', 'archived')", name='valid_status')
    )

    op.create_index('idx_tasks_status', 'tasks', ['status'], postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_tasks_project', 'tasks', ['project_id'], postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_tasks_scheduled', 'tasks', ['scheduled_date'], postgresql_where=sa.text('deleted_at IS NULL'))
    op.create_index('idx_tasks_blocked_by', 'tasks', ['blocked_by_task_id'], postgresql_where=sa.text('deleted_at IS NULL'))
