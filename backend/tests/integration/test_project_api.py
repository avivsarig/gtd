...
class TestProjectAPI:
    ...
    def test_create_project(self, client_postgres: TestClient, db_session_postgres: Session):
    ...
    def test_create_project_minimal(self, client_postgres: TestClient):
        """Should create project with only name."""
        response = client_postgres.post("/api/v1/projects/", json={"name": "Minimal Project"})
    ...
    def test_create_project_empty_name_returns_422(self, client_postgres: TestClient):
        """Should reject empty project name."""
        response = client_postgres.post("/api/v1/projects/", json={"name": ""})

        assert response.status_code == 422

    def test_list_projects_empty(self, client_postgres: TestClient):
        """Should return empty list when no projects exist."""
        response = client_postgres.get("/api/v1/projects/")

        assert response.status_code == 200
        assert response.json() == []

    def test_list_projects(self, client_postgres: TestClient):
        """Should return all projects."""
        # Create multiple projects
        client_postgres.post("/api/v1/projects/", json={"name": "Project 1"})
        client_postgres.post("/api/v1/projects/", json={"name": "Project 2"})
        client_postgres.post("/api/v1/projects/", json={"name": "Project 3"})
    ...
    def test_list_projects_with_stats(self, client_postgres: TestClient):
        ...
        response = client_postgres.get("/api/v1/projects/?with_stats=true")
    ...
    def test_get_project_by_id(self, client_postgres: TestClient):
        ...
        response = client_postgres.get(f"/api/v1/projects/{project_id}")
    ...
    def test_get_project_not_found(self, client_postgres: TestClient):
        """Should return 404 when project doesn't exist."""
        fake_uuid = "00000000-0000-0000-0000-000000000000"
        response = client_postgres.get(f"/api/v1/projects/{fake_uuid}")
    ...
    def test_update_project_partial(self, client_postgres: TestClient):
    ...
    def test_update_project_status(self, client_postgres: TestClient):
        """Should update project status."""
        # Create project
        create_response = client_postgres.post("/api/v1/projects/", json={"name": "Project"})
        project_id = create_response.json()["id"]

        # Update to on_hold
        response = client_postgres.put(f"/api/v1/projects/{project_id}", json={"status": "on_hold"})
    ...
    def test_complete_project(self, client_postgres: TestClient):
        ...
        # Complete project
        response = client_postgres.post(f"/api/v1/projects/{project_id}/complete")
    ...
    def test_delete_project(self, client_postgres: TestClient):
        ...
        # Delete project
        delete_response = client_postgres.delete(f"/api/v1/projects/{project_id}")

        assert delete_response.status_code == 204

        # Verify project is not in list
        list_response = client_postgres.get("/api/v1/projects/")
        projects = list_response.json()
        assert not any(p["id"] == project_id for p in projects)

        # Verify project returns 404
        get_response = client_postgres.get(f"/api/v1/projects/{project_id}")
        assert get_response.status_code == 404

    def test_delete_project_not_found(self, client_postgres: TestClient):
        """Should return 404 when deleting non-existent project."""
        fake_uuid = "00000000-0000-0000-0000-000000000000"
        response = client_postgres.delete(f"/api/v1/projects/{fake_uuid}")
