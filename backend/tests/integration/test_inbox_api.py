...
class TestInboxCRUD:
    ...
    def test_create_inbox_item(self, client_postgres: TestClient, db_session_postgres: Session):
        """Should create inbox item with minimal data."""
        response = client_postgres.post("/api/v1/inbox/", json={"content": "Random thought"})
    ...
    def test_create_inbox_item_without_content_returns_422(self, client_postgres: TestClient):
        """Should reject inbox item creation without content."""
        response = client_postgres.post("/api/v1/inbox/", json={})

        assert response.status_code == 422

    def test_list_inbox_items(self, client_postgres: TestClient, db_session_postgres: Session):
        """Should list all unprocessed inbox items."""
        # Create some inbox items
        client_postgres.post("/api/v1/inbox/", json={"content": "First thought"})
        client_postgres.post("/api/v1/inbox/", json={"content": "Second thought"})
    ...
    def test_get_inbox_item_by_id(self, client_postgres: TestClient, db_session_postgres: Session):
        ...
        # Get by ID
        response = client_postgres.get(f"/api/v1/inbox/{item_id}")
    ...
    def test_get_nonexistent_inbox_item_returns_404(self, client_postgres: TestClient):
        """Should return 404 for nonexistent inbox item."""
        fake_id = uuid4()
        response = client_postgres.get(f"/api/v1/inbox/{fake_id}")
    ...
    def test_update_nonexistent_inbox_item_returns_404(self, client_postgres: TestClient):
        """Should return 404 when updating nonexistent item."""
        fake_id = uuid4()
        response = client_postgres.put(f"/api/v1/inbox/{fake_id}", json={"content": "Won't work"})

        assert response.status_code == 404

    def test_delete_inbox_item(self, client_postgres: TestClient, db_session_postgres: Session):
        """Should soft delete inbox item."""
        # Create inbox item
        create_response = client_postgres.post("/api/v1/inbox/", json={"content": "Delete me"})
        item_id = create_response.json()["id"]

        # Delete
        delete_response = client_postgres.delete(f"/api/v1/inbox/{item_id}")

        assert delete_response.status_code == 204

        # Verify item is gone (soft deleted)
        get_response = client_postgres.get(f"/api/v1/inbox/{item_id}")
        assert get_response.status_code == 404

    def test_delete_nonexistent_inbox_item_returns_404(self, client_postgres: TestClient):
        """Should return 404 when deleting nonexistent item."""
        fake_id = uuid4()
        response = client_postgres.delete(f"/api/v1/inbox/{fake_id}")
...
class TestInboxCount:
    ...
    def test_get_unprocessed_count(self, client_postgres: TestClient, db_session_postgres: Session):
        """Should return count of unprocessed items."""
        # Create some inbox items
        client_postgres.post("/api/v1/inbox/", json={"content": "Item 1"})
        client_postgres.post("/api/v1/inbox/", json={"content": "Item 2"})
        client_postgres.post("/api/v1/inbox/", json={"content": "Item 3"})
...
class TestInboxConversions:
    ...
    def test_convert_inbox_to_task_with_defaults(self, client_postgres: TestClient, db_session_postgres: Session):
        ...
        convert_response = client_postgres.post(f"/api/v1/inbox/{item_id}/convert-to-task", json={})
        ...
        inbox_response = client_postgres.get("/api/v1/inbox/")
    ...
    def test_convert_nonexistent_inbox_to_task_returns_404(self, client_postgres: TestClient):
        """Should return 404 when converting nonexistent inbox item."""
        fake_id = uuid4()
        response = client_postgres.post(f"/api/v1/inbox/{fake_id}/convert-to-task", json={})
    ...
    def test_convert_inbox_to_note_with_defaults(self, client_postgres: TestClient, db_session_postgres: Session):
        ...
        # Convert to note
        convert_response = client_postgres.post(f"/api/v1/inbox/{item_id}/convert-to-note", json={})
    ...
    def test_convert_nonexistent_inbox_to_note_returns_404(self, client_postgres: TestClient):
        """Should return 404 when converting nonexistent inbox item to note."""
        fake_id = uuid4()
        response = client_postgres.post(f"/api/v1/inbox/{fake_id}/convert-to-note", json={})
    ...
    def test_convert_nonexistent_inbox_to_project_returns_404(self, client_postgres: TestClient):
        """Should return 404 when converting nonexistent inbox item to project."""
        fake_id = uuid4()
        response = client_postgres.post(f"/api/v1/inbox/{fake_id}/convert-to-project", json={})
...
class TestInboxProcessedFilter:
    ...
    def test_list_excludes_processed_by_default(self, client_postgres: TestClient, db_session_postgres: Session):
        ...
        item_id = create_response.json()["id"]
        client_postgres.post(f"/api/v1/inbox/{item_id}/convert-to-task", json={})

        # List inbox items (should not include processed)
        list_response = client_postgres.get("/api/v1/inbox/")
    ...
    def test_list_includes_processed_when_requested(self, client_postgres: TestClient, db_session_postgres: Session):
        ...
        list_response = client_postgres.get("/api/v1/inbox/?include_processed=true")
        items = list_response.json()
        item_ids = [item["id"] for item in items]
    ...
    def test_count_excludes_processed_items(self, client_postgres: TestClient, db_session_postgres: Session):
        ...
        item2 = client_postgres.post("/api/v1/inbox/", json={"content": "Item 2"}).json()
        client_postgres.post("/api/v1/inbox/", json={"content": "Item 3"})

        # Process two of them
        client_postgres.post(f"/api/v1/inbox/{item1['id']}/convert-to-task", json={})
        client_postgres.post(f"/api/v1/inbox/{item2['id']}/convert-to-note", json={})

        # Count should be 1
        count_response = client_postgres.get("/api/v1/inbox/count")
