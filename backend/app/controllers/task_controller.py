...
class TaskController(BaseController[Task, TaskRepositoryProtocol]):
    """Controller for Task entity with business logic."""
    ...
    def create_task(self, db: Session, task_data: TaskCreate) -> Task:
        ...
        return self.repository.create(db, task_data)

    def update_task(self, db: Session, task_id: UUID, task_data: TaskUpdate) -> Task | None:
        ...
        return self.repository.update(db, task, task_data)

    def delete_task(self, db: Session, task_id: UUID) -> Task | None:
        ...
        return self.repository.soft_delete(db, task)

    def _update_task_field(
        self, db: Session, task_id: UUID, field_name: str, field_value: datetime | None
    ) -> Task | None:
        ...
        # Transaction management - commit and refresh
        db.commit()
        db.refresh(task)
        return task

    def complete_task(self, db: Session, task_id: UUID) -> Task | None:
        ...
        """
        return self._update_task_field(db, task_id, "completed_at", datetime.now(UTC))

    def uncomplete_task(self, db: Session, task_id: UUID) -> Task | None:
        ...
        """
        return self._update_task_field(db, task_id, "completed_at", None)

    def bulk_update_status(
        self, db: Session, task_ids: list[UUID], status: TaskStatus
    ) -> list[Task]:
        ...
        # Transaction management - single commit for all updates
        db.commit()
