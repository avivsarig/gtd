...
class SearchRepository:
    """Repository for full-text search operations across multiple entity types."""
    ...
    def search_all(self, db: Session, query: str, limit: int = 50) -> list[dict]:
        ...
        task_results = self._build_search_query(db, Task, "task", tsquery)

        note_results = self._build_search_query(db, Note, "note", tsquery)

        project_results = self._build_search_query(
            db, Project, "project", tsquery, include_project_id=False
        )
